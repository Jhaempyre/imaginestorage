...
import { FileIcon, FileSpreadsheetIcon, FileTextIcon, LayoutDashboard } from "lucide-react";
...
const getDocumentIcon = (mimeType: string) => {
  ...
  if (mimeType.includes('sheet') || mimeType.includes('excel')) return FileSpreadsheetIcon;
  if (mimeType.includes('presentation') || mimeType.includes('powerpoint')) return LayoutDashboard;
...
const getDocumentColor = (mimeType: string) => {
  ...
  if (mimeType.includes('sheet') || mimeType.includes('excel')) return 'text-green-500 bg-green-50';
  ...
  return (
    ...
    >
      {document.previewUrl && !imageError ? (
        ...
        <div className={`w-full h-full flex flex-col items-center justify-center ${colorClasses}`}>
      ...
      <div className="absolute inset-0 group-hover:bg-black/20 transition-all duration-200 flex items-end">
        <div className="w-full p-2 text-white text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-200 truncate bg-gradient-to-t from-black/60 to-transparent">
