...
export class UserStorageConfig {
  ...
  /**
   * @deprecated Use encryptedCredentials instead
   */
  @Prop({
    required: true,
    type: Object,
  ...
  @Prop({
    required: true,
    type: String, // Changed to String to store encrypted data
    select: false, // Don't include credentials in default queries for security
  })
  encryptedCredentials: string = "__EMPTY__"; // Renamed from 'credentials'
...
export const UserStorageConfigSchema =
  SchemaFactory.createForClass(UserStorageConfig);

// Indexes for better query performance
UserStorageConfigSchema.index({ userId: 1, isActive: 1 });
UserStorageConfigSchema.index({ provider: 1, isValidated: 1 });

// Instance method to get decrypted credentials (will be injected by service)
UserStorageConfigSchema.methods.getDecryptedCredentials = function(): StorageCredentials {
  // This will be implemented in the service layer
  throw new Error('getDecryptedCredentials must be called through the service');
};

// Instance method to mask sensitive credentials
UserStorageConfigSchema.methods.getMaskedCredentials =
  function (): Partial<StorageCredentials> {
    const masked: Partial<StorageCredentials> = {};

    switch (this.provider) {
      case "aws":
        masked.accessKeyId = this.credentials.accessKeyId?.replace(
          /(.{4}).*(.{4})/,
          "$1***$2",
        );
...
UserStorageConfigSchema.methods.validateRequiredFields = function (): {
  isValid: boolean;
