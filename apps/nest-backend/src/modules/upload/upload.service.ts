...
export class UploadService {
  ...
  async generateUploadToken(publicKey: string, origin: string): Promise<UploadTokenResponseDto> {
    try {
      ...
      this.logger.log(`Generated upload token for user ${user._id} with API key ${apiKey._id}`);
      ...
      this.logger.error(`Failed to generate upload token for public key ${publicKey}:`, error);
      throw error;
    }
  }
  ...
  async uploadFileWithToken(
    ...
    try {
      ...
      // Upload to storage
      const uploadResult = await this.storageService.uploadFile(payload.userId, {
      ...
      try {
        ...
        this.logger.warn(`Failed to cleanup temporary file ${file.path}:`, cleanupError);
      }

      this.logger.log(`File uploaded successfully via widget: ${uniqueFileName} for user ${payload.userId}`);
